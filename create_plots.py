...
class Informations:
    ...
    def change_axis(self, column_name, quarterly):
        df_balance_sheet_income = self.get_df(column_name)
        date_axis = []
    ...
    def create_plot(self, column_name, quarterly):
        ...
        if len(x_axis) == len(y_axis):
            ...
            print(f'No data! len(x_axis) != len(y_axis)')
            return False

    def default_sheet(self, default_location, column_name, quarterly=None):
        os.chdir(default_location)
        data_true_or_false = self.create_plot(column_name, quarterly)
        if data_true_or_false:
            ...
            print(f'Image saved! -> {self.company_ticker}_{self.get_info}_{column_name}.png')
        else:
